...
```
<draggable v-model="myTeachers" group="people" @start="drag=true" @end="drag=false" class="drag-box" >
   <span v-for="element in myTeachers" :key="element.id+element.name">{{element.name}}</span>
</draggable>
```
