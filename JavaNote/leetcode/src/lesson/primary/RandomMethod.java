 ...
 * 随机数
 *
 * 1. 首先使用现有的随机函数,创造一个等概率返回0和1的随机函数
 * 2. 因为我们现在已经可以等概率得到0和1,所以现在我们只需要通过二进制位来得到我们想要的范围
 * 3. 如果是三位二进制数,我们就可以随机得到0到7等概率的随机数
 * 4. 这里通过二进制对0和1进行位移操作我们就可以产生二进制数    (a() << 2) + (a() << 1) + a()  这里三位二进制数
 * 5. 通过二进制得到的等概率数,只需要通过一些简单的方法就可以得到我们想要的范围内的随机数
 *
 *
 */
